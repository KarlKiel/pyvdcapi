...
def test_announcer_init():
    ...
    announcer = ServiceAnnouncer(port=8444, host_name="test-host", use_avahi=False)

    assert announcer.port == 8444
    assert announcer.host_name == "test-host"
    assert announcer.use_avahi is False
...
def test_announcer_properties():
    ...
    announcer = ServiceAnnouncer(port=9999, host_name="custom-vdc", use_avahi=False)
...
def test_zeroconf_availability():
    ...
    try:
        ...
        pytest.skip("zeroconf library not installed (optional)")
