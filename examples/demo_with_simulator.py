...
from examples.vdsm_simulator import VdsmSimulator  # noqa: E402
...
async def create_demo_setup():
    ...
    logger.info("✓ Created vDC: Smart Lights Controller")
    ...
    living_room_light.add_output_channel(
        channel_type=DSChannelType.BRIGHTNESS, name="brightness", min_value=0.0, max_value=100.0, resolution=1.0
    )
    ...
    bedroom_light.add_output_channel(channel_type=DSChannelType.BRIGHTNESS, name="brightness")
    bedroom_light.add_output_channel(channel_type=DSChannelType.HUE, name="hue")
    bedroom_light.add_output_channel(channel_type=DSChannelType.SATURATION, name="saturation")
    ...
    kitchen_sensor.add_sensor(sensor_type="temperature", name="Temperature", unit="°C", min_value=-20.0, max_value=60.0)
    ...
    logger.info("\n✓ Setup complete:")
    logger.info("  - 2 vDCs")
    logger.info("  - 3 devices (2 lights, 1 sensor)")
        ...
        if simulator.devices:
            ...
            if light_device:
                ...
                await simulator.set_output_value(
                    light_device.dsuid, DSChannelType.BRIGHTNESS, 50.0, transition_time=1.0
                )
                ...
                await simulator.set_output_value(
                    light_device.dsuid, DSChannelType.BRIGHTNESS, 100.0, transition_time=2.0
                )
