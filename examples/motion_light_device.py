...
async def main():
    ...
    # Create vDC host
    host = VdcHost(name="Motion Light Controller", port=8440, persistence_file="motion_light.yaml")

    # Create vDC for light devices
    vdc = host.create_vdc(vendor="Example Inc", model_name="Smart Motion Light", model_guid="example-motion-light-v1")

    # Create virtual device (smart light)
    device = vdc.create_vdsd(
        name="Living Room Light", group=DSGroup.LIGHT, model="ML-100", vendor="Example", serial="ML100-001"
    )
    ...
    motion_detector = BinaryInput(
        vdsd=device, name="Motion Detector", input_type="motion", input_id=0, initial_state=False
    )
