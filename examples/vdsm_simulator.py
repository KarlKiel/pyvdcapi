...
class VdsmSimulator:
    ...
    async def send_hello(self) -> bool:
        ...
        logger.info("→ Sending Hello (dSUID: vdsm-simulator-001)")
    ...
    async def discover_vdcs(self) -> List[str]:
        ...
        msg.vdsm_request_generic_request.params.extend(PropertyTree.to_protobuf({"x-p44-vdcs": None}))
    ...
    async def discover_devices(self, vdc_dsuid: str) -> List[str]:
        ...
        msg.vdsm_request_generic_request.params.extend(PropertyTree.to_protobuf({"x-p44-vdsds": None}))
    ...
    async def set_output_value(self, dsuid: str, channel_type: int, value: float, transition_time: float = 0.0):
        """
...
async def main():
    ...
    parser.add_argument(
        "--scenario", choices=["discovery", "full_test"], default="discovery", help="Test scenario to run"
    )
