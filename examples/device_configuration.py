...
RGB_LIGHT_TEMPLATE = {
    ...
    "buttons": [{"name": "Power Button", "buttonType": "toggle", "element": 0}],
    "sensors": [
        {
    ...
    "scenes": {
        DSScene.PRESENT: {
            "channels": {DSChannelType.BRIGHTNESS: 75.0, DSChannelType.HUE: 0.0, DSChannelType.SATURATION: 0.0},
            "effect": DSSceneEffect.SMOOTH,
        },
        DSScene.ABSENT: {"channels": {DSChannelType.BRIGHTNESS: 0.0}, "effect": DSSceneEffect.SMOOTH},
        DSScene.SLEEPING: {
            "channels": {DSChannelType.BRIGHTNESS: 10.0, DSChannelType.HUE: 30.0, DSChannelType.SATURATION: 80.0},
...
MOTION_SENSOR_TEMPLATE = {
    "binary_inputs": [{"name": "Motion Detector", "inputType": "motion", "invert": False, "initialState": False}],
    "sensors": [
        {
...
async def main():
    ...
    # Create vDC host
    host = VdcHost(name="Configuration Demo", port=8440, persistence_file="config_demo.yaml")

    # Create vDC
    vdc = host.create_vdc(vendor="Example Inc", model_name="Configurable Devices", model_guid="example-config-demo-v1")
    ...
    rgb_light = vdc.create_vdsd(
        name="Living Room RGB Light", group=DSGroup.LIGHT, model="RGB-100", vendor="Example", serial="RGB100-001"
    )
    ...
    kitchen_light = vdc.create_vdsd(
        name="Kitchen Light", group=DSGroup.LIGHT, model="RGB-100", vendor="Example", serial="RGB100-003"
    )
    ...
    bedroom_config["binary_inputs"] = [
        {"name": "Motion Detector", "inputType": "motion", "invert": False, "initialState": False}
    ]

    # Add panic scene
    bedroom_config["scenes"][DSScene.PANIC] = {
        "channels": {DSChannelType.BRIGHTNESS: 100.0, DSChannelType.HUE: 0.0, DSChannelType.SATURATION: 100.0},
    ...
    motion_sensor = vdc.create_vdsd(
        name="Hallway Motion Sensor", group=DSGroup.SECURITY, model="MS-50", vendor="Example", serial="MS50-001"
    )
    ...
    office_light = vdc.create_vdsd(
        name="Office RGB Light", group=DSGroup.LIGHT, model="RGB-100", vendor="Example", serial="RGB100-003"
    )
    ...
    print("All devices configured from templates")
    print("Configuration saved to YAML: config_demo.yaml")
