...
class TCPServer:
    ...
    async def _handle_client_connection(
        ...
        if self._current_connection is not None:
            logger.warning(
                f"Rejecting connection from {peername} - "
                f"already connected to another vdSM"
            )
            writer.close()
        ...
        while self._running:
            try:
                ...
                if message_length > MAX_MESSAGE_SIZE:
                    logger.error(
                        f"Received message with invalid length {message_length}, "
                        f"max is {MAX_MESSAGE_SIZE}"
                    )
                ...
                if self.message_handler:
                    try:
                        ...
                        logger.error(
                            f"Error in message handler for message type {message.type}: {e}",
                            exc_info=True
                        )
                else:
