...
from ..core.constants import DSChannelType, DSSceneEffect, get_channel_name
...
class OutputChannel:
    ...
    def set_value(
        ...
        validated_value = self._validator.clamp_value(
            value,
            self.min_value,
            self.max_value
        )
        ...
        logger.debug(
            f"Channel {self.name} value changed: {old_value} → {validated_value}, "
            f"effect={effect}"
        )
    ...
    def update_value(self, value: float) -> None:
        ...
        if validated_value != self._value:
            ...
            logger.debug(
                f"Channel {self.name} updated from hardware: "
                f"{old_value} → {validated_value}"
            )
    ...
    def from_dict(self, data: Dict[str, Any]) -> None:
        ...
        if 'state' in data and 'value' in data['state']:
            self.set_value(data['state']['value'])
        elif 'value' in data:
            # Support direct value property for convenience
