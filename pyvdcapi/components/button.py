"""
Button - LEGACY timing-based button implementation.

⚠️ DEPRECATION NOTICE:
   This class is maintained for backward compatibility but is NOT API-compliant.
   For new implementations, use ButtonInput instead.

   Issue: This class calculates clickType from press/release timing.
   API Spec: clickType should be an integer INPUT value (vDC API section 4.2.3).

   Recommended migration:
     OLD: button = device.add_button(name="Light", button_type=1)
     NEW: button = device.add_button_input(name="Light", button_type=1)
          state_machine = DSButtonStateMachine(button)
          hardware.on_press(state_machine.on_press)
          hardware.on_release(state_machine.on_release)

   See: examples/button_input_example.py for complete examples.

A Button represents a physical or virtual button/switch that generates
events when activated by a user. This implementation uses timing-based
pattern detection which does not match the vDC API specification.

Button Types and Events:
┌──────────────────────────────────────────────────────────┐
│ Button Event Types (LEGACY TIMING-BASED)                │
├──────────────────────────────────────────────────────────┤
│ 0: Single Press                                          │
...
class Button:
    ...
    - Double press: Two quick taps (within 500ms by default)
    - Long press: Hold for >1 second (by default)
    - Release: Released after long press

    Custom Timing:
    Timing constants can be customized per instance:

        # Fast double-press detection for responsive UI
        button = Button(vdsd=device, name="Quick Button", button_type=0)
        button.DOUBLE_PRESS_INTERVAL = 0.3  # 300ms window

        # Longer hold required for safety-critical actions
        button = Button(vdsd=device, name="Emergency Stop", button_type=2)
        button.LONG_PRESS_THRESHOLD = 2.0  # 2 second hold
    ...
    Attributes:
        ...
        name: Button name
        DOUBLE_PRESS_INTERVAL: Max time between presses (default: 0.5s)
        LONG_PRESS_THRESHOLD: Min hold time for long press (default: 1.0s)
    """
